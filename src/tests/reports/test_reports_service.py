...
def test_create_report_reuses_existing_report_for_same_idempotency_key(uow):
    """
    ...
    reports_before = uow.session.query(Report).count()
    jobs_before = uow.session.query(Job).count()
    events_before = uow.session.query(OutboxEvent).count()

    report1 = create_report(
        uow, idempotency_key=key, request_id="req-1", submit_job=submit_job
    )
    report2 = create_report(
        uow, idempotency_key=key, request_id="req-2", submit_job=submit_job
    )

    uow.commit()

    reports_after = uow.session.query(Report).count()
    jobs_after = uow.session.query(Job).count()
    events_after = uow.session.query(OutboxEvent).count()
...
def test_create_report_rolls_back_when_job_attachment_fails(uow, monkeypatch):
...
def test_complete_report_sets_ready_and_result(uow, get_report):
    """Completing a report should mark it ready and persist the result payload."""
    report = _create_report_row(uow.session, idempotency_key=None)
    uow.commit()

    result = {"url": "s3://bucket/x.pdf"}
    complete_report(uow, report_id=report.id, result=result)
    uow.commit()
...
def test_complete_report_invalid_state_raises(uow):
    """Completing a report twice should raise InvalidReportState."""
    report = _create_report_row(uow.session, idempotency_key=None)
    uow.commit()

    complete_report(uow, report_id=report.id, result={"ok": True})
    uow.commit()
...
def test_complete_report_missing_report_raises(uow):
    """Completing a missing report should raise ReportNotFound."""
    with pytest.raises(ReportNotFound):
        complete_report(uow, report_id="missing", result={"ok": True})
